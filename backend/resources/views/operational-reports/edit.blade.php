    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <form action="{{ route('operational-reports.update', $report->report_id) }}"
                    method="POST">
                        ...
                        <select name="assignment_id"
                            ...
                            @foreach ($assignments as $assignment)
                                <option value="{{ $assignment->assignment_id }}"
                                    {{ old('assignment_id', $report->assignment_id) == $assignment->assignment_id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description"
                            id="description"
                            rows="4"
                            class="w-full border-gray-300 rounded-lg shadow-sm">{{ old('description', $report->description) }}</textarea>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select name="status"
                            ...
                            @foreach (['Draft', 'Submitted', 'Approved', 'Rejected'] as $status)
                                <option value="{{ $status }}"
                                    {{ old('status', $report->status) == $status ? 'selected' : '' }}>
                                    {{ $status }}
                                </option>
                            @endforeach
